...
function CreatePanel:ChooseWidget()
    local isLeader = IsGroupLeader()
    local isCreated = self:IsActivityCreated()
    local showCreate = isLeader or not isCreated

    -- 队伍每次有人进出都会跑到这,原来是无条件Hide一遍再SetShown,
    -- 隐藏那一下会把标题输入框的键盘焦点和输入法一起顶掉,所以只在状态真的不一样时才动它
    if self.CreateWidget:IsShown() ~= showCreate then
        self.CreateWidget:SetShown(showCreate)
    end
    if self.ViewBoardWidget:IsShown() ~= (not showCreate) then
        self.ViewBoardWidget:SetShown(not showCreate)
    end
end
