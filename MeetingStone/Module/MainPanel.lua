    ...
    for i = 1, numMembers do
        ...
        if ok and m and type(m.name) == 'string' and m.name ~= '' then
            ...
            if dashPos then
                -- 名字自带服务器: 精确查, 查不到就是不是
                found = p[m.name] ~= nil
            elseif m.isLeader then
                -- 队长没带服务器: 先看info.leaderName能不能推出服务器, 推不出就当他跟自己同服
                local realm = (info.leaderName and info.leaderName:match('^([^%-]+)%-(.+)$')) or MyRealmName()
                found = realm and p[m.name .. '-' .. realm] ~= nil or false
            else
                -- 普通队员拿不到服务器: 只按名字匹配(名单键是"名字-服务器", 比"-"之前那段, 要求全等)
                for k in pairs(p) do
                    if (k:match('^([^%-]+)%-') or k) == m.name then
                        found = true
                        break
                    end
                end
